...
def load_glove_w2v(word_map):
    print("loading word2vec...")
    glove_path = "/users5/yjtian/Downloads/glove.840B.300d.w2v.txt"
    t0 = time.time()
    if os.path.exists("./data/glove.txt"):
        print("load glove.txt")
        w2v = gensim.models.KeyedVectors.load_word2vec_format("./data/glove.txt")
    else:
        w2v = gensim.models.KeyedVectors.load_word2vec_format(glove_path)
    t1 = time.time()
    print("gensim load glove time: {:.1f} s".format(t1 - t0))   # 645s
    print("\nEmbedding length is %d.\n" % w2v.vector_size)
    ...
    for word in word_map:
        if word in w2v.vocab:
            embeddings[word_map[word]] = torch.FloatTensor(w2v[word])
            words[word] = w2v.word_vec(word)

    print("find word time: {:.1f} s".format(time.time() - t1))  # 3s
    print("Embedding vocabulary: %d, in w2v: %d(%.4f).\n" % (
    len(word_map), len(words), float(len(words)) / len(word_map)))

    if not os.path.exists("./data/glove.txt"):
        print("write glove vector to ./data/glove.txt")
        with open("./data/glove.txt", "w", encoding="utf-8") as f:
